...
void Sprite2D::HandleTouchEvents(GLint x, GLint y, bool bIsPressed)
{
	if (!bIsPressed)
	{
		m_isHandle = false;
...
bool Sprite2D::IsHandle()
{
	return m_isHandle;
}

bool Sprite2D::checkTexture(std::string texture)
{
	return(m_pTexture == ResourceManagers::GetInstance()->GetTexture(texture));
}

void Sprite2D::changeTexture(std::string nextTexture)
{
